...
async def main():
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        resp = await client.get("http://localhost:8085/health")
        print("Health Response:", resp.json())

        # Generate a dummy video and image for testing
        os.system("ffmpeg -f lavfi -i color=c=blue:s=100x100 -vframes 10 dummy_video.mp4 -y")
        os.system("ffmpeg -f lavfi -i color=c=red:s=100x100 -vframes 1 dummy_image.png -y")

        print("Testing POST /api/v1/detect (Image only)")
        with open("dummy_image.png", "rb") as f:
            resp = await client.post("http://localhost:8085/api/v1/detect", files={"file": ("dummy_image.png", f, "image/png")})
            print("Detect Image:", resp.json())

        print("Testing POST /api/v1/detect (Video only)")
        with open("dummy_video.mp4", "rb") as f:
            resp = await client.post("http://localhost:8085/api/v1/detect", files={"video": ("dummy_video.mp4", f, "video/mp4")})
        ...
        with open("dummy_video.mp4", "rb") as f, open("dummy_image.png", "rb") as ch, open("dummy_image.png", "rb") as nm:
            resp = await client.post("http://localhost:8085/api/v1/detect", 
                files={
